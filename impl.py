...
from .jordan_block import jordan_blocks_odd, jordan_blocks_2, JordanBlock2, JordanBlocks
...
def _siegel_series_polynomial_2(q):
    ...
    unit_diags_first_blc = [
        qf for expt, qf in q.blocks if expt == max_expt and qf not in non_diags
    ]
    ...
    if len(unit_diags_first_blc) == 1:
        ...
        return cbb2_1(q, q2) * pol.subs({X: ZZ(2) * X}) + cbb2_0(q, q2) * pol
    ...
    times2 = {X: ZZ(2) * X}
    times4 = {X: ZZ(4) * X}
    coeffs = [c11 * c21.subs(times2), c11 * c20.subs(times2) + c10 * c21, c10 * c20]
    pol = _siegel_series_polynomial_2(q2)
...
def _siegel_series_polynomial_odd(q, p):
    ...
    return cbb2_1(q, q2) * pol.subs({X: p * X}) + cbb2_0(q, q2) * pol
...
def siegel_series_dim2(q, p):
    ...
    return _siegel_series_dim2(p, c, f + 1) - kronecker_symbol(
        fd, p
    ) * p * X * _siegel_series_dim2(p, c, f)
