...
class PullBackTest(unittest.TestCase):
    def assert_pullback_m_1(self, k, A1, prec=10):
        ...
        for a in range(prec):
            self.assertEqual(
                f[a], eisenstein_pullback_coeff(k, A1, diagonal_matrix([a]))
            )
    ...
    def assert_pullback_m_2(self, k, A1, prec=3):
        ...
        for t in PrecisionDeg2(prec):
            self.assertEqual(
                f[t], eisenstein_pullback_coeff(k, A1, tpl_to_half_int_mat(t))
            )
    ...
    def test_pullback_3_1(self):
        ...
        self.assert_pullback_m_1(6, matrix([[1, 1, 0], [1, 1, 0], [0, 0, 2]]))
